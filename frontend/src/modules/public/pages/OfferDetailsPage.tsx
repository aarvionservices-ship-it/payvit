...
import { ArrowLeft, CheckCircle2, Percent, Banknote, Calendar, CreditCard, FileText, Info, MapPin, User, Briefcase, FileImage, Loader2 } from 'lucide-react';
...
export default function OfferDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50">
      <SEO 
        title={`${loan.loanName} from ${loan.bankName}`} 
        description={`Apply for ${loan.loanName} at ${loan.interestRate?.min}% interest rate. Get instant approval and flexible tenure options with PayVit.`}
      />
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <Link to="/offers" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors group">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl shadow-slate-200/40 overflow-hidden relative">
          ...
          <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
            ...
            <Link to="/login" className="relative z-10 w-full md:w-auto text-center px-8 py-3.5 text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-600/20 hover:shadow-blue-600/40 transition-all hover:-translate-y-0.5">
              Apply Now
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-slate-100 border-b border-slate-100 bg-gradient-to-b from-slate-50/80 to-white">
            ...
            <div className="p-6 text-center group hover:bg-white transition-colors">
              ...
              <p className="text-xl font-bold text-slate-900">{formatCompactNumber(loan.loanAmount?.max || 0)}</p>
            </div>
            <div className="p-6 text-center group hover:bg-white transition-colors">
          ...
          <div className="p-6 md:p-8 space-y-10">
            ...
            <section>
              ...
              <ul className="grid sm:grid-cols-2 gap-4">
                ...
                <li className="flex items-start gap-3 bg-slate-50 rounded-xl p-4 border border-slate-100 hover:border-emerald-200 transition-colors group">
                  ...
                  <span className="text-slate-700 font-medium mt-1">Minimum net monthly income of {formatCurrency(loan.eligibility?.minimumIncomeMonthly?.salaried || 25000)}.</span>
                </li>
                <li className="flex items-start gap-3 bg-slate-50 rounded-xl p-4 border border-slate-100 hover:border-emerald-200 transition-colors group">
                  <div className="bg-emerald-100 rounded-full p-2 mt-0.5 shrink-0 group-hover:scale-110 transition-transform">
            ...
            <section>
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                <FileText className="size-5 text-indigo-500" /> Required Documents
              </h2>
              <ul className="grid sm:grid-cols-2 gap-4">
