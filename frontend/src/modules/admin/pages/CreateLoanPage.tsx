...
export default function CreateLoanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.loanName || !formData.bankName || !formData.provider) {
      toast.error('Core identity vectors are incomplete');
      return;
    }
    ...
    try {
      ...
      const response = await createLoan(submitData);
      if (response.success) {
        toast.success('Asset deployed to master portfolio');
        navigate('/admin/loans');
      } else {
        toast.error(response.message || 'Deployment verification failed');
      }
    } catch (error) {
      console.error('Error creating loan:', error);
      toast.error('System logic synchronization failure');
    } finally {
    ...
    <div className="p-6 max-w-[1400px] mx-auto space-y-10 pb-32">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900 leading-none tracking-tight uppercase">Provision Asset Logic</h1>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mt-2">Comprehensive marketplace parameter initialization</p>
          </div>
        </div>
        <button 
           ...
           <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                 ...
                 <div className="space-y-2 p-4 bg-slate-50 rounded-2xl">
                    ...
                    <input type="number" value={formData.eligibility?.workExperience?.totalYears} onChange={e => handleDeepInputChange(['eligibility', 'workExperience', 'totalYears'], parseInt(e.target.value))} className="w-full bg-white border-none rounded-xl text-center font-black text-indigo-600 py-3 shadow-sm text-lg" />
                 </div>
                 <div className="space-y-2 p-4 bg-slate-50 rounded-2xl">
